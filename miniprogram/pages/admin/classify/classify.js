...
Page({
  ...
  formClassifySubmit: async function (e) {
    ...
    if (classifyName === undefined || classifyName === "") {
      ...
      let res = await api.addBaseClassify(classifyName, classifyDesc, app.globalData.openid)
      wx.hideLoading()
      if (res.result) {
        that.setData({
          isClassifyModelShow: false,
  ...
  deleteClassifyById: async function (e) {
    ...
    wx.showModal({
      ...
      success(res) {
        if (res.confirm) {
          api.deleteConfigById(classifyId, app.globalData.openid).then(res => {
  ...
  savePostsRelatedClassify: async function (e) {
    ...
    try {
      let res = await api.updateBatchPostsClassify(that.data.curClassifyName, that.data.tabCur == 1 ? "add" : "delete", posts, app.globalData.openid)
      if (res.result) {
        wx.showToast({
          title: '处理成功',
          icon: 'none',
