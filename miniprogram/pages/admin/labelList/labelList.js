...
Page({
  ...
  onLoad: async function (options) {
    await this.getLabelList(app.globalData.openid)
  },
  ...
  formLabelSubmit: async function (e) {
    ...
    if (labelName === undefined || labelName === "") {
      ...
      let res = await api.addBaseLabel(labelName, app.globalData.openid)
      wx.hideLoading()
      if (res.result) {
        that.setData({
          isLabelModelShow: false,
  ...
  savePostsRelatedLabel: async function (e) {
    ...
    try {
      let res = await api.updateBatchPostsLabel(that.data.curLabelName, that.data.tabCur == 1 ? "add" : "delete", posts, app.globalData.openid)
      if (res.result) {
        wx.showToast({
          title: '处理成功',
          icon: 'none',
