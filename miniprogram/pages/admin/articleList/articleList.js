...
Page({
  ...
  saveLabelModal: async function (e) {
    ...
    try {
      ...
      let res = await api.upsertPosts(postId === undefined ? "" : postId, newPost, app.globalData.openid)
      if (res.result) {
        that.setData({
          isShowLabelModel: false,
          selectedLabels: [],
  ...
  deleteShowModal: async function (e) {
    ...
    try {
      let that = this;
      let res = await api.deletePostById(that.data.showCurPostId, app.globalData.openid)
      if (res.result) {
        that.setData({
          isShowDeleteModel: false,
          showCurPostId: "",
